...
| 会话复制与分支 | 复制完整会话，或从指定消息位置创建副本；Codex 另支持模型服务分支 | 支持 | 支持会话复制 | 支持会话复制 | 不支持 |
...
会话卡片上的「复制会话」会复制全部消息；预览中的「复制到此处」会按消息顺序复制到所选内容所属的整条消息，包含这条消息的全部文字、思考、工具和其他内容块。副本保存在同一项目下，标题添加 ` (fork #1)`，再次复制副本会递增编号。可从副本卡片复制 `opencode --session <新会话 ID>` 命令继续对话。
...
<details>
<summary>Codex App 开着时可以删除、移动或恢复会话吗？</summary>

可以。不会仅因 Codex App 正在运行就禁止操作。删除、移动、归档、覆盖导入/恢复和本地分支操作会检查实际受影响的会话：短暂比较所选会话的文件及线程记录，发现写入变化时停止操作并弹窗提示先停止该会话的生成或等待完成，不要求退出 App。检查不会为判断活跃状态扫描其他会话；分页会话还会检查线程记录当前指向的续写文件。

没有观察到写入不代表 agent 已停止：等待工具或网络时可能暂时没有新记录。因此仍保留提交前检查、文件指纹比对及事务处理；操作时请勿同时恢复目标会话的生成。App 运行期间不改写它缓存的私有项目配置，已打开的列表可能需要重启后刷新；移动等操作的 Desktop 项目归属是否同步会单独提示。
