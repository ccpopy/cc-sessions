...
Codex 归档视图会按归档来源分组显示：手动归档、切换模型服务时自动归档、备份恢复或会话包导入产生的迁移记录、以及来源不明的记录；可以用顶部筛选条只看某一类。导出页会保留归档记录供手动选择，并显示“已归档”徽标。Codex 使用本地归档目录保存会话文件，OpenCode 使用自身的原生归档状态。
...
| 补全归档来源标记 | 旧版归档会话没有来源记录，归档视图无法分组 | 只写 CC Sessions 自己的来源记录，不改会话文件 |
...
Codex 归档视图会按归档来源分组：手动归档、切换模型服务时自动归档、备份恢复或会话包导入产生的迁移记录，以及来源不明的记录。来源记录保存在 CC Sessions 自己的数据里，不会改动会话文件。旧版本升级后已有的归档可能没有来源记录，可以在修复工具里用“补全归档来源标记”补齐。
